...
# temporary in‑memory store for recommendations
...
def _process_recommendations(recommended_df: pd.DataFrame) -> List[Dict[str, Any]]:
    # build maps for scores, fuzzy→clean→raw→menus→restaurants→locations→hours
    ...
    raw_ids       = clean_subset["raw_dish_id"].unique().tolist()

    menu_subset = menu_df[menu_df["raw_dish_id"].isin(raw_ids)]
    ...
    rest_ids = menu_subset["restaurant_id"].dropna().unique().tolist()
    ...
    for _, r in business_hours_df[business_hours_df["restaurant_id"].isin(rest_ids)].iterrows():
    ...
    for dish_name in rec_list:
        clean_ids_for_dish = dish_to_clean.get(dish_name, [])
        raw_ids_for_dish   = [rid for cid in clean_ids_for_dish
                              for rid in clean_to_raw.get(cid, [])]

        restaurants_list = []
        for rwid in raw_ids_for_dish:
            for rec in raw_lookup.get(rwid, []):
                ...
                restaurants_list.append({
                    ...
                    "latitude": loc_lookup.get(rid, {}).get("latitude"),
                    "longitude": loc_lookup.get(rid, {}).get("longitude"),
                    "address": loc_lookup.get(rid, {}).get("address"),
                    "city": loc_lookup.get(rid, {}).get("city"),
                    "state": loc_lookup.get(rid, {}).get("state"),
...
def show_recommendations():
    ...
    # Build folium map of all restaurant markers
    points = [
        (
            rest["latitude"],
            rest["longitude"],
            f"<b><a href='#{'rec_'+dish['dish_name_clean_fuzzy'].replace(' ','_')}'>" 
            f"{rest['restaurant_name']}</a></b>"
        )
        for dish in final_list
        for rest in dish["restaurants"]       # ← iterate over dish["restaurants"]
    ...
    if points:
        ...
        map_html = "<p>No restaurant locations available.</p>"

    return render_template(
        "recommendations.html",
        grouped_recs=final_list,
        liked_dishes=liked,
        map_html=map_html
    )
